...
class SuperAdminDashboardService
{
    ...
    public static function userStats(){
        $userStats = User::with(['roles','roles.permissions']);

        if(isset($role) && $role!=''){
            $userStats = $userStats->with([
        ...
        $userStats = $userStats->where('id','!=',1)->orderBy('created_at','DESC')->get();

        return $userStats;
    }

    public static function clinicStats(){
        $clinicStats = Clinic::where('id','!=',1)->orderBy('created_at','DESC')->get();
        return $clinicStats;
    }
